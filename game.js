/**
 * HYPERDRIVE PRO — Full Engine
 * 20-car roster · Coin economy · Milestones · Garage UI
 * Nitro · Near-miss · Combo · Difficulty · Rain · Touch controls
 */
...
// ═══════════════════════════════════════════════════════════════════════════
// CAR ROSTER  (20 cars across 5 tiers)
// Each car has a drawFn hook so you can drop in your own pixel-art designs.
// drawFn(ctx, x, y, w, h, { nitroBoosting, gameSpeed }) — called every frame.
// Leave drawFn: null to use the built-in gradient renderer.
// ═══════════════════════════════════════════════════════════════════════════

// Convert stat values (1-10) into actual game runtime values


// ═══════════════════════════════════════════════════════════════════════════
// MILESTONES  (one-time coin bonuses)
// ═══════════════════════════════════════════════════════════════════════════
const MILESTONES = [
...
let ownedCarIds   = LS.get('hd_owned',  ['city_cruiser']);
let selectedCarId = LS.get('hd_selected','city_cruiser');
...
const player = {
  ...
  width: 40, height: 76, targetX: canvas.width / 2 - 20
};
...
function refreshHighScore() {
  const record = LS.get(storageKey(), 0);
  if (menuHighScore) menuHighScore.textContent = record.toLocaleString();
...
function renderToggles() {
  [menuToggle, gameOverToggle].forEach(container => {
    if (!container) return;
    container.innerHTML = ['SAME_WAY','OPPOSITE'].map(k => {
      const active = chosenMode === k ? 'active-mode' : '';
      const label = k === 'SAME_WAY' ? 'SAME-WAY FLOW' : 'HEAD-ON OPPOSITE';
      const desc  = k === 'SAME_WAY' ? 'Overtake traffic in your direction.' : 'Survive high-speed oncoming traffic.';
      return `<button class="mode-btn ${active}" data-mode="${k}" type="button">
        <span class="mode-title">${label}</span><span class="mode-desc">${desc}</span>
      </button>`;
    }).join('');
  });
  document.querySelectorAll('.mode-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      chosenMode = btn.dataset.mode; refreshHighScore(); renderToggles();
    });
  });
}
...
function buildGarage() {
  ...
  CAR_ROSTER.forEach((car, i) => {
    ...
    drawMiniCar(mini.getContext('2d'), car, 10, 7, 40, 76);
  ...
  selectGarageIndex(Math.max(0, CAR_ROSTER.findIndex(c => c.id === selectedCarId)));
}

function selectGarageIndex(i) {
  garageIndex = Math.max(0, Math.min(CAR_ROSTER.length - 1, i));
  const cardW = 92; // card width + gap
  garageTrack.style.transform = `translateX(${-(garageIndex * cardW) + 110}px)`;
  document.querySelectorAll('.garage-card').forEach((c, idx) => c.classList.toggle('active-card', idx === garageIndex));
  updateCarInfoBox();
}
...
function updateCarInfoBox() {
  ...
  if (sel) {
    ...
    carActionBtn.onclick     = () => { selectedCarId = car.id; saveEconomy(); buildGarage(); };
  } else {
    const canAfford = wallet >= car.price;
    ...
    carActionBtn.onclick     = canAfford ? () => {
      wallet -= car.price; ownedCarIds.push(car.id); saveEconomy(); refreshWallet(); buildGarage();
    } : null;
  }
}
...
// Mini car render for garage cards
function drawMiniCar(c, car, x, y, w, h) {
  if (car.drawFn) { car.drawFn(c, x, y, w, h, {}); return; }
  const g = c.createLinearGradient(x, y, x+w, y);
  g.addColorStop(0, shadeColor(car.color, -30));
  g.addColorStop(0.5, car.color);
  g.addColorStop(1, shadeColor(car.color, -20));
...
function update() {
  ...
  if (nitroKey && nitro > 0 && !nitroRecharge) {
    ...
    nitroBoosting = false;
    if (nitroRecharge) {
      nitro = Math.min(nitroMax, nitro + 0.4);
      if (nitro >= nitroMax * 0.3) nitroRecharge = false;
    } else {
      nitro = Math.min(nitroMax, nitro + 0.2);
    }
  }
  updateNitroBar();

  const speedMult  = nitroBoosting ? activeRuntime.nitroMult : 1.0;
  const modeMult   = chosenMode === 'OPPOSITE' ? 1.6 : 1.0;
  const diffMult   = chosenDiff === 'INSANE' ? 1.3 : chosenDiff === 'EASY' ? 0.8 : 1.0;
  const comboBonus = 1 + (combo - 1) * 0.1;
  score += gameSpeed * 0.06 * modeMult * diffMult * speedMult * comboBonus;
  ...
  const lateralSpd = activeRuntime.lateralSpeed;
  if (touchActive) { const delta = touchCurrentX - touchStartX; player.targetX += delta * 0.22; touchStartX = touchCurrentX; }
  if (keys['a'] || keys['ArrowLeft'])  player.targetX -= lateralSpd;
  if (keys['d'] || keys['ArrowRight']) player.targetX += lateralSpd;
  const wallL = roadX + 12, wallR = roadX + roadWidth - player.width - 12;
  player.targetX = Math.max(wallL, Math.min(wallR, player.targetX));
  player.x += (player.targetX - player.x) * 0.2;

  spawnExhaust();

  trackLines.forEach(l => { l.y += gameSpeed * speedMult; if (l.y > canvas.height) l.y = -80; });
  roadMarkers.forEach(m => { m.y += gameSpeed * speedMult; if (m.y > canvas.height + 50) m.y -= canvas.height + 200; });
  ...
  if (obstacles.length === 0 || (obstacles[obstacles.length-1].y > 180 && Math.random() < diff.spawnChance)) {
    ...
    const vy = chosenMode === 'SAME_WAY'
      ? (Math.random() * 2.5 - 5.0) * diff.obsSpeedMult
      : (Math.random() * 3.0 + 2.0) * diff.obsSpeedMult;
...
function draw() {
  ...
  for (let g = 0; g < roadWidth; g += 3) {
    ...
    const yo = Math.floor(score * gameSpeed * 0.3) % 40;
  ...
  const ru = Math.floor(score * 2.2) % 28;
  ...
  if (gameState === 'PLAYING' || gameState === 'PAUSED') drawPlayerCar();
  ...
  if (nitroBoosting) {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      ctx.beginPath(); ctx.moveTo(sx, 0); ctx.lineTo(sx + (Math.random()-0.5)*20, canvas.height); ctx.stroke();
    }
    ctx.restore();
  }
}
...
function drawPlayerCar() {
  ...
  const car = activeCar;
  if (car.drawFn) { car.drawFn(ctx, x, y, w, h, { nitroBoosting, gameSpeed }); return; }

  ctx.save();
  ctx.shadowBlur = nitroBoosting ? 32 : 18;
  ctx.shadowColor = nitroBoosting ? car.accentColor : car.color;
  ctx.fillStyle = (nitroBoosting ? car.accentColor : car.color) + '44';
  ctx.fillRect(x - 6, y - 6, w + 12, h + 12);
  ctx.restore();

  ctx.fillStyle = 'rgba(0,0,0,0.35)'; ctx.fillRect(x + 4, y + 6, w, h);
  ctx.fillStyle = '#0c0c18'; ctx.fillRect(x - 2, y, w + 4, h);

  const bg = ctx.createLinearGradient(x, y, x + w, y);
  bg.addColorStop(0, shadeColor(car.color, -30));
  bg.addColorStop(0.4, car.color);
  bg.addColorStop(1, shadeColor(car.color, -20));
  ctx.fillStyle = bg; ctx.fillRect(x, y + 3, w, h - 6);

  ctx.fillStyle = shadeColor(car.color, -20); ctx.fillRect(x + 4, y + 3, w - 8, 16);
  ctx.fillStyle = '#0a0a14'; ctx.fillRect(x + 6, y + 20, w - 12, 22);
  ctx.fillStyle = 'rgba(255,255,255,0.06)'; ctx.fillRect(x + 8, y + 22, w - 16, 4);
  ctx.fillStyle = '#081428'; ctx.fillRect(x + 5, y + 24, w - 10, 16);
  ctx.fillStyle = car.accentColor + '33'; ctx.fillRect(x + 7, y + 26, 6, 8);
  ctx.fillStyle = '#081428'; ctx.fillRect(x + 5, y + h - 32, w - 10, 14);
  ctx.fillStyle = car.accentColor + 'aa'; ctx.fillRect(x + w/2 - 1, y + 3, 2, h - 6);

  ctx.save();
  ctx.shadowBlur = nitroBoosting ? 18 : 10; ctx.shadowColor = car.accentColor;
  ctx.fillStyle = nitroBoosting ? '#ffffff' : '#e0f0ff';
  ctx.fillRect(x + 3, y + 3, 7, 5); ctx.fillRect(x + w - 10, y + 3, 7, 5);
  ctx.restore();

  ctx.save();
  ctx.shadowBlur = 8; ctx.shadowColor = '#ff0040'; ctx.fillStyle = '#ff2244';
  ctx.fillRect(x + 3, y + h - 7, 8, 5); ctx.fillRect(x + w - 11, y + h - 7, 8, 5);
  ctx.restore();

  drawWheel(x - 3, y + 10, 6, 14); drawWheel(x + w - 3, y + 10, 6, 14);
  drawWheel(x - 3, y + h - 28, 6, 14); drawWheel(x + w - 3, y + h - 28, 6, 14);
}

// ── Traffic car ───────────────────────────────────────────────────────────
function drawTrafficCar(obs) {
  const { x, y, width: w, height: h, color } = obs;
  const oncoming = chosenMode === 'OPPOSITE';
  ctx.fillStyle = 'rgba(0,0,0,0.28)'; ctx.fillRect(x+3, y+5, w, h);
  ctx.fillStyle = '#0a0a14'; ctx.fillRect(x-1, y, w+2, h);
  const g = ctx.createLinearGradient(x, y, x+w, y);
  g.addColorStop(0, shadeColor(color,-40)); g.addColorStop(0.5, color); g.addColorStop(1, shadeColor(color,-30));
  ctx.fillStyle = g; ctx.fillRect(x, y+3, w, h-6);
  ctx.fillStyle = shadeColor(color,-20);
  ctx.fillRect(x+3, oncoming ? y+h-18 : y+3, w-6, 15);
  ctx.fillStyle = '#0c0c18'; ctx.fillRect(x+5, y+(oncoming ? h-40 : 20), w-10, 20);
  ctx.fillStyle = '#0d1a24'; ctx.fillRect(x+6, y+(oncoming ? h-38 : 22), w-12, 14);
  ctx.save();
  if (oncoming) {
  ...
  ctx.restore();
  drawWheel(x-3, y+10, 6, 12); drawWheel(x+w-3, y+10, 6, 12);
  drawWheel(x-3, y+h-24, 6, 12); drawWheel(x+w-3, y+h-24, 6, 12);
}

// ── Draw helpers ──────────────────────────────────────────────────────────
function drawWheel(x, y, w, h) {
  ctx.fillStyle = '#050508'; ctx.fillRect(x, y, w, h);
  ctx.fillStyle = '#222230'; ctx.fillRect(x+1, y+2, w-2, h-4);
  ctx.fillStyle = '#3a3a50'; ctx.fillRect(x+2, y+h/2-1, w-4, 2);
}
...
function shadeColor(hex, amt) {
  let c = parseInt((hex.replace('#','')).padStart(6,'0'), 16);
  return `rgb(${Math.max(0,Math.min(255,(c>>16)+amt))},${Math.max(0,Math.min(255,((c>>8)&0xff)+amt))},${Math.max(0,Math.min(255,(c&0xff)+amt))})`;
}
